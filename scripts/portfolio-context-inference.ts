...
export const inferTools = (portfolioItem: PortfolioContextInput): string[] => {
  ...
  if (text.includes('ai ') || text.includes(' ai') || text.includes('sentiment')) tools.add('AI-enabled analysis concepts');
  return [...tools];
};
...
export const inferApplicationHint = (portfolioItem: PortfolioContextInput) => {
  ...
  if (text.includes('assessment') || text.includes('rubric') || text.includes('post-test') || text.includes('feedback')) return 'Made learning outcomes and participant experience easier to measure, compare, and improve.';
  if (text.includes('proposal') || text.includes('business plan') || text.includes('marketing plan')) return 'Turned learning ideas into a clearer plan for stakeholder alignment, delivery, and business decision-making.';
  if (text.includes('workbook') || text.includes('worksheet') || text.includes('template')) return 'Gave learners and facilitators practical tools for reflection, planning, and follow-through.';
  if (text.includes('bootcamp') || text.includes('program') || text.includes('seminar') || text.includes('workshop')) return 'Supported applied learning through structured activities, facilitation, and practical implementation tasks.';
